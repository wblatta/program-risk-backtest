...
def test_fires_when_no_activity_in_N_weeks():
    ...
    assert item_silent({"x:i": s}, ctx(datetime(2024, 6, 1, tzinfo=UTC))) == {("x:i", "alpha")}

def test_quiet_when_recent_activity():
    s = item(last_activity=datetime(2024, 5, 20, tzinfo=UTC))
    assert item_silent({"x:i": s}, ctx(datetime(2024, 6, 1, tzinfo=UTC))) == set()

def test_fires_when_never_active():
    assert item_silent({"x:i": item()}, ctx(datetime(2024, 6, 1, tzinfo=UTC))) == {("x:i", "alpha")}
